...
| `host/rust/` | Rust CLI host (`iotsploit-host workflow ble-connect-pair`) |
...
```bash
cd /home/tkxb/Projects/esp32s3_demo/iotsploit-usb/examples/esp32s3
sudo iotsploit-host workflow ble-scan                        # 先看设备列表/索引
sudo iotsploit-host workflow ble-connect-pair                # 连接+配对
```
